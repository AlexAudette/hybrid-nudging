...
! Alexandre Audette 2021/03/08
      use lw_prescribed_mod, only : fbot_delta, rside_delta
...
!
      ...
      !real (kind=dbl_kind), dimension(nx_block,ny_block) :: rside_temp
!=======================================================================
...
!
      ...
      ! ice nudging variables
      real (kind=dbl_kind), dimension (nx_block,ny_block), &
         intent(in) :: &
         fbot_delta, & ! bottom heat flux nudging
         rside_delta   ! lateral nudging fraction
...
!
      ...
      real (kind=dbl_kind) :: &
         ...
         dhi_lat       ! volume difference due to lateral melt. (Alexandre Audette 2021)
...
! ech: the size of these arrays should be reduced to icells
      ...
      do ij = 1, icells
         ...
         if (n .eq. 1) then
            !-----------------------------------------------------------------
            ! Compute fluxes of water and salt added to the ocean when nudging
            ! the ice to a specific state/
            !-----------------------------------------------------------------
            dhi_lat = (hin(ij)-dhi)*rside_delta(i,j)*dt
         
              
            freshn(i,j) = evapn(i,j) - &
                       (rhoi*(dhi+dhi_lat) + rhos*(dhs-hsn_new(ij))) / dt
            fsaltn(i,j) = -rhoi*(dhi+dhi_lat)*ice_ref_salinity*p001/dt
         endif
         freshn(i,j) = evapn(i,j) - &
...
!ocl novrec      !Fujitsu
      ...
      do ij = 1, icells
         ...
         wk1 = (fcondbot(ij) - fbot(i,j)+fbot_delta(i,j)) * dt
...
!
      ...
      real (kind=dbl_kind), dimension(nx_block,ny_block), &
           intent(in) :: rside_delta ! fraction of ice melted due to nudging algorithm.
!
!EOP
!
      integer (kind=int_kind) :: &
...
!ocl novrec      !Fujitsu
      do ij = 1, icells
         i = indxi(ij)
         j = indxj(ij)
         !if (rside_delta(i,j) .lt. puny) then
         if (n .eq. 1) then !add ice to the thinnest ice category
            aicen(i,j) = min(c1,max(c0,(aicen(i,j) - &
                 rside_delta(i,j)*dt)))
         end if
         !else
     
            !aicen(i,j) = min(c1,(aicen(i,j) - &
            !     rside_delta(i,j)*dt)) ! remove proportionnaly from all categories
            
            !write(nu_diag,*) 'fraction of ice in category ncat = ', ncat, 'ice fraction = ', aicen(i,j)/aice(i,j)

...
!       end if
    ...
    !-----------------------------------------------------------------
    ! Prescribed lw initialization
    !-----------------------------------------------------------------

    call lw_prescribed_init(ICEID, gsmap_ice, dom_i)
...
! BT 6 August 2012
! try calling prescibed_lw here - use the cice_presc timers?

    if(prescribed_lw) then  ! read prescribed long wave
       call t_startf ('cice_presc')
       call lw_prescribed_run(idate, sec)
